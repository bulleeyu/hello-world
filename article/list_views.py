...
def article_titles(request, username=None):
    if username:
        ...
        all_articles = ArticlePost.objects.filter(author=user)
        try:
...
def article_detail(request, id, slug):
    ...
    r.zincrby('article_ranking', 1, article.id)  # (如果没有就创建一个)字典'article_ranking',以article_id为键，每次访问加1
    article_ranking = r.zrange('article_ranking', 0, -1, desc=True)[:10]  # 将'article_ranking'字典按value排序，截取前10个。
